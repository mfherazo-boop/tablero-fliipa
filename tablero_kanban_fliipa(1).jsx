...
export default function KanbanBoard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) loadedTasks = parsed;
      } catch (e) {
        /* la clave nueva puede no existir todavía */
      }
      if (!loadedTasks) {
        // Migración: si no hay nada bajo la clave nueva, busca datos guardados
        // bajo el nombre anterior (mal escrito) y los adopta.
        try {
          const legacy = await window.storage.get(TASKS_KEY_LEGACY, true);
          const parsedLegacy = legacy && legacy.value ? JSON.parse(legacy.value) : null;
          if (Array.isArray(parsedLegacy) && parsedLegacy.length > 0) {
            loadedTasks = parsedLegacy;
            setWithRetry(TASKS_KEY, JSON.stringify(parsedLegacy), true);
          }
        } catch (e) {
          /* no había datos legacy */
        }
      }
      setTasks(loadedTasks || seedTasks());
      setLoaded(true);
      try {
        let theme = await window.storage.get(THEME_KEY, true);
        if (!theme || !theme.value) {
          theme = await window.storage.get(THEME_KEY_LEGACY, true).catch(() => null);
          if (theme && theme.value) setWithRetry(THEME_KEY, theme.value, true);
        }
        if (theme && theme.value) setDark(theme.value === "dark");
      } catch (e) {
        /* keep default */
      }
      try {
        const result = await window.storage.get(INITIATIVES_KEY, true);
        const parsed = result && result.value ? JSON.parse(result.value) : null;
        setInitiatives(Array.isArray(parsed) ? parsed : []);
      } catch (e) {
        setInitiatives([]);
      }
    })();
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      setSaving(true);
      const { ok, error: err } = await setWithRetry(TASKS_KEY, JSON.stringify(tasks), true);
      // Si mientras reintentábamos llegó un cambio más nuevo, ese guardado
      // ya se encargará de reflejar el estado correcto; no pisamos su resultado.
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setTasks((prev) => [...prev]); // vuelve a disparar el efecto de guardado
    }, 8000);
  ...
  useEffect(() => {
    ...
    setWithRetry(THEME_KEY, dark ? "dark" : "light", true);
  }, [dark, loaded]);
  ...
  useEffect(() => {
    ...
    initSaveTimer.current = setTimeout(async () => {
      setSavingInit(true);
      const { ok, error: err } = await setWithRetry(INITIATIVES_KEY, JSON.stringify(initiatives), true);
  ...
  function addInitiative({ title, owner, progress }) {
    const now = Date.now();
    setInitiatives((prev) => [{ id: "i" + now, title, owner, progress: Number(progress) || 0, createdAt: now }, ...prev]);
  ...
  function adjustInitiativeProgress(id, delta) {
    setInitiatives((prev) =>
      prev.map((i) => (i.id === id ? { ...i, progress: Math.max(0, Math.min(100, i.progress + delta)) } : i))
    );
  }
  ...
  function addTask(task) {
    const now = Date.now();
    setTasks((prev) => [{ ...task, id: "t" + now, createdAt: now, statusChangedAt: now }, ...prev]);
    setModalOpen(false);
  }
  ...
  function toggleBlocked(id) {
    setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, blocked: !t.blocked } : t)));
  }
            ...
            >
              <div style={{ color: C.textMuted, fontSize: 13.5, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <span>
                  ...
                  {filtered.length !== 1 ? "s" : ""} · compartido con quien tenga este link
                </span>
                {tasks.length === 0 && (
                  <button
